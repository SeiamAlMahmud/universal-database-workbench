...
const SchemaItem: React.FC<{ node: any; level: number }> = ({ node, level }) => {
    ...
    return (
        <div className="select-none">
            <div
                className={`flex items-center gap-1.5 px-3 py-1 hover:bg-white/5 cursor-pointer text-xs
          ${isOpen ? "text-slate-200" : "text-slate-400"}`}
            ...
            >
                ...
                {node.metadata?.dataType && (
                    <span className="text-slate-600 ml-auto">{node.metadata.dataType}</span>
                )}
            </div>
            {isOpen && hasChildren && (
                <div>
                    {node.children.map((child: any) => (
                        <SchemaItem key={child.id} node={child} level={level + 1} />
                    ))}
                </div>
            )}
        </div>
        ...
        >
            ...
            {showAddDialog && (
                <div className="m-2 p-3 bg-slate-900 rounded-lg border border-slate-700">
                    <p className="text-xs text-slate-400 mb-2 font-medium">New SQLite Connection</p>
                    <input
                        type="text"
            ...
            <div className="flex-1 overflow-y-auto py-1">
              ...
              ${activeConnectionId === conn.id
                        ...
                        >
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-slate-600 truncate">
                                    {conn.filename || conn.host || conn.type}
                                </p>
                            </div>
                        ...
                        {/* Schema Tree */}
                        {activeConnectionId === conn.id && schemas[conn.id] && (
                            <div className="pb-2 border-b border-slate-800/50">
                                {schemas[conn.id].map((node) => (
                                    <SchemaItem key={node.id} node={node} level={0} />
                                ))}
                            </div>
                        )}
                    </div>
